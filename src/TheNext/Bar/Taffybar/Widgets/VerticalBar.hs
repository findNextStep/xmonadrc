-- | copy from System.Taffybar.Widgets.VerticalBar in taffybar stable-0.5
-- can see in https://github.com/travitch/taffybar/blob/stable-0.5/src/System/Taffybar/Widgets/VerticalBar.hs
-- | 从taffybar 项目中复制并稍加修改

-- | A vertical bar that can plot data in the range [0, 1].  The
-- colors are configurable.
module TheNext.Bar.Taffybar.Widgets.VerticalBar (
  -- * Types
  VerticalBarHandle,
  BarConfig(..),
  BarDirection(..),
...
verticalBarNew cfg = do
  ...
  case barDirection cfg of
    ...
    HORIZONTAL  -> widgetSetSizeRequest drawArea 64 (barWidth cfg)
